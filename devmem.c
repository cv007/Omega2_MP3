   ...
   for(int i = 0, idx = 0, argn = 1; i < argc; idx++){
        ...
        switch(cmd){
            ...
            case 'c':
                ...
                addr = strto_u32(argv[i++], argn++);
                val = strto_u32(argv[i++], argn++);
                break;
            default:
                arg_error_exit(argn, argv[i-1]);
                break;
        }
...
void regread(char opt, uint32_t addr, uint32_t val)
{
    ...
    if(opt == 'v'){
        ...
        printf(
            ...
            "dec %15d  %15d  %15d  %15d\n\n",
            " ", val_u.b3, " ", val_u.b2, " ", val_u.b1, " ", val_u.b0, val_u.b3, val_u.b2, val_u.b1, val_u.b0);
    } else if(opt == 'h'){
        printf("0x%08X\n", val);
    } else if(opt = 'b'){
        printf("0b ");
...
static void usage(const char* n){
    printf(
        ...
        "    can provide more than one command at a time, up to a limit of 255 arguments\n"
        "\n",
        n,n,n,n,n,n
    );
    exit(EXIT_FAILURE);
...
int main(int argc, char** argv)
{
    ...
    for(int i = 0; g_cmdbuffer[i].cmd; i++){
        cmd_buffer_t cb = g_cmdbuffer[i];
        switch(cb.cmd){
            case 'r': printf( "%d\n", *(regmap(cb.addr)) );     break;
            case 'h': printf( "0x%08X\n", *(regmap(cb.addr)) ); break;
            case 'b':
                cb.val = *(regmap(cb.addr));
                printf("0b ");
                for(int j = 0; j < 32; j++, cb.val<<=1){
